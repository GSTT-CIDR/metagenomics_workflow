...
VIRAL_SCORE = int(snakemake.config["viral_score"])
...
def main():
    ...
    df = df[(df["taxID"] > 0) & (df["score"] >= VIRAL_SCORE)]
    if df.shape[0] != 0:
        ...
        del report_values[9606] # Remove human reads
        ...
        report_df.to_csv(TARGETS_OUTPUT, index=False, sep="\t")
